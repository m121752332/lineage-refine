...
`invSize`＝背包容量（預設 100，向歐林購買每次 +100 格花 1000 萬天幣，上限 1000）。

### `lineage_inventory`

背包陣列（長度＝`lineage_player.invSize`，初始 100 格），陣列索引即格子位置：
...
背包工具列勾選「整批衝裝」後，點武器／防具卷軸開啟三階段視窗（詳見 [`docs/adr/0003`](docs/adr/0003-batch-refine-and-inventory-expansion.md)）：

1. **待處理**：載入背包中所有同類裝備，點選／SHIFT 範圍多選。
2. **指定等級**：下拉選 +1 ~ 最高強化上限（`maxEnhLevel`）。
3. **衝裝處理**：輪巡逐件精煉（每 500ms 一次、拉霸式高亮），每次嘗試消耗 1 張同變體卷軸、**免強化費**；機率與單把精煉相同（安定範圍 100%，失敗爆裝化為粉末）。卷軸用盡即中止。結束顯示總結，「回到背包」收回存活裝備。

> 詛咒卷軸不可用於整批（成功會降級，永遠衝不到目標）；祝福 +2 跳級會 clamp 在目標等級。
...
- 初始 100 格（5 欄），可向歐林購買擴充（+100 格／1000 萬天幣，上限 1000 格），支援依類型 / 名稱 / 強化等級排序
- 強化裝備顯示格式統一為「+N 名稱」（如 `+10 大馬士革刀`）
- **鐵鎚模式**：點卷軸啟動 → 再點裝備格即自動強化，適合批量操作
- **整批衝裝**：工具列勾選後點卷軸，多選裝備一次衝到指定等級（見上方精煉系統）
- 右鍵快速操作：直接強化 / 直接出售
